...
class DTensorExportTest(TestCase):
    ...
    def test_flex_attention_dtensor_export(self):
        ...
        def _block_mask_unflatten(tensors, context):
        ...
        def _block_mask_flatten_with_keys(block_mask: BlockMask):
            """Flatten BlockMask with keys for better tracing."""
            tensors = [
                (GetAttrKey("kv_num_blocks"), block_mask.kv_num_blocks),
                (GetAttrKey("kv_indices"), block_mask.kv_indices),
                (GetAttrKey("full_kv_num_blocks"), block_mask.full_kv_num_blocks),
                (GetAttrKey("full_kv_indices"), block_mask.full_kv_indices),
                (GetAttrKey("q_num_blocks"), block_mask.q_num_blocks),
                (GetAttrKey("q_indices"), block_mask.q_indices),
                (GetAttrKey("full_q_num_blocks"), block_mask.full_q_num_blocks),
                (GetAttrKey("full_q_indices"), block_mask.full_q_indices),
            ]
            context = (
                block_mask.seq_lengths,
                block_mask.BLOCK_SIZE,
        ...
        register_pytree_node(
            ...
            flatten_with_keys_fn=_block_mask_flatten_with_keys,
